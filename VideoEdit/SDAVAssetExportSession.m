    ...
    while (input.isReadyForMoreMediaData)
    {
        ...
        if (sampleBuffer)
        {
            ...
            if (!handled && self.videoOutput == output)
            {
                ...
                if ([self.delegate respondsToSelector:@selector(exportSession:renderFrame:withPresentationTime:toBuffer:)] && CMTimeGetSeconds(lastSamplePresentationTime) < 9)
                {
                    CVPixelBufferRef pixelBuffer = (CVPixelBufferRef)CMSampleBufferGetImageBuffer(sampleBuffer);
                    CVPixelBufferRef renderBuffer = NULL;
                    CVPixelBufferPoolCreatePixelBuffer(NULL, self.videoPixelBufferAdaptor.pixelBufferPool, &renderBuffer);
                    [self.delegate exportSession:self renderFrame:pixelBuffer withPresentationTime:lastSamplePresentationTime renderBuffer:&renderBuffer];
